...
export const securityControls: readonly SecurityControl[] = [
	...
	{
		id: 'trusted-types',
		title: 'Trusted Types (enforced)',
		category: 'web',
		status: 'shipped',
		what: 'The enforcing `Content-Security-Policy` carries `require-trusted-types-for \'script\'; trusted-types default unifi-demo`. DOM-XSS sinks (`innerHTML`, `Element.outerHTML`, `createContextualFragment`, Worker URLs, etc.) throw unless their value is minted by an allowlisted Trusted Types policy.',
		...
		tradeoffs: 'Enforcing — unwrapped sink writes throw rather than logging. The Astro-emitted bundles carry no Trusted Types injection sinks: DOM is built via `createElement`/`textContent`, lists are cleared with `replaceChildren()`, and the one build-time-trusted HTML string (shiki highlights) is parsed with `DOMParser` rather than `innerHTML`. The single legitimate bundle sink — the PoW Worker URL spawned by `mail-pow.ts` — is wrapped by the fail-closed `default` policy, which only mints same-origin `/_astro/` script URLs and throws on everything else. The standalone `/apps/unifi-demo` asset renders its own static markup through `innerHTML`, routed through the scoped, named `unifi-demo` createHTML policy (allowlisted alongside `default` in the directive); its `createHTML` is not a passthrough — it validates every string against the demo\'s known-safe shape (a fixed presentational tag allowlist, no event-handler attributes, no `javascript:` URIs) and throws otherwise, so a future edit that interpolated untrusted markup would fail closed rather than become DOM-XSS. If the directive ever drops the name, the demo fails visibly rather than silently.',
		code: [
			'infra/cloudfront.tf',
			'src/scripts/util/trusted-types.ts',
			'src/scripts/mail-pow.ts',
	...
	},
	{
		...
		tradeoffs: '`mode: enforce` (`max_age: 604800`, RFC 8461 §3.2 SHOULD floor) on millsymills.com: senders that respect MTA-STS refuse delivery on policy mismatch, promoted from testing after a clean TLS-RPT soak showing `policy-type: sts` — see `docs/superpowers/specs/2026-05-14-millsymills-mail-activation-design.md` § Future. Reversal is asymmetric: publish `mode: none` AND wait `max_age` BEFORE removing the discovery TXT, otherwise enforcing senders refuse delivery during the rollback window.',
	...
	},
	{
		...
		category: 'identity',
		status: 'shipped',
		what: 'Branch protection on `main` requires a verified signature on every commit (`required_signatures.enabled = true`, `enforce_admins = true`); GitHub rejects an unsigned push with `GH006: Protected branch update failed -- Commits must have verified signatures`. The rule is enforced server-side via the `default-branch-protection` repository ruleset plus out-of-band `required_signatures`, both managed in the GitHub UI (deliberately not Terraform-managed, so a `github_branch_protection_*` resource cannot fight the live ruleset); `deploy.yml` reads `branches/main/protection/required_signatures` at build time and fails the deploy on drift. `CONTRIBUTING.md` documents the SSH signing setup contributors run once: reuse the GitHub auth key, `git config gpg.format ssh`, register a Signing Key in GitHub Settings, verify with `git log --show-signature`.',
		...
		code: ['CONTRIBUTING.md', '.github/workflows/deploy.yml'],
		prs: [321],
	},
	{
		id: 'mail-pow',
