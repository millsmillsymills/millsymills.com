...
export const securityControls: readonly SecurityControl[] = [
	...
	{
		id: 'csp-script-hashes',
		title: 'Strict CSP `script-src` with build-time SHA-256 hashes',
		category: 'web',
		status: 'shipped',
		what: 'A postbuild step (`scripts/inject-csp-hashes.mjs`) computes the SHA-256 of every JS bundle under `dist/_astro/`, then injects a `<meta http-equiv="Content-Security-Policy">` tag into each HTML\'s `<head>` that pins `script-src` to those hashes plus `\'strict-dynamic\'`. Reports flow to the existing `/api/csp-report` endpoint via both `report-uri` (legacy) and `report-to csp` (Reporting API) directives.',
		why: 'The header CSP (provisioned at terraform-apply time) cannot rotate per deploy, so it stays as the broad enforcing fallback (`script-src \'self\'`). The meta CSP layers tighter `\'sha256-…\'` constraints on top of every static HTML response — browsers apply the intersection, so a script must be both same-origin (per header) AND in the build-time-known hash set (per meta) to execute. Closes the XSS-via-bundle-substitution surface that `\'self\'` alone leaves open. The `\'strict-dynamic\'` keyword lets entry-point scripts transitively load further `/_astro/` chunks without each chunk needing its own hash.',
		tradeoffs: 'The original #129 framing was per-request nonces via a CloudFront / Lambda@Edge function. CloudFront Functions do not have access to response bodies (see [function event structure](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/functions-event-structure.html)), so per-request nonce injection is not possible without Lambda@Edge plus its per-request cost. Build-time hashes deliver the equivalent restriction (every executable script is build-time-known) with zero new runtime infra. The meta CSP regenerates on every build, so site-deploy and CSP-update ship in the same artifact.',
		code: ['scripts/inject-csp-hashes.mjs', 'package.json', 'src/data/security-controls.ts'],
	},
	{
		id: 'trusted-types',
