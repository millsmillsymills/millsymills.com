...
export const securityControls: readonly SecurityControl[] = [
	...
	},
	{
		...
		tradeoffs: 'Cost cap is `reserved_concurrent_executions = 5` on the Lambda — a flood of reports gets throttled rather than driving up the bill. Unlike the other Lambda endpoints, this Function URL is public (`authorization_type = NONE`), not CloudFront-OAC-locked: OAC SigV4 can\'t carry a browser-supplied POST body, so an OAC-fronted report endpoint returns 403 for every browser report. It is a write-only, unauthenticated sink — 204 with no body, Content-Type/size/concurrency capped, no read or enumerate path — so a publicly reachable URL exposes no data, though a caller can submit well-formed reports, making captured violations advisory rather than trusted. No dashboard yet — reports are queryable directly out of the S3 bucket via Athena or `aws s3 cp` until violation volume justifies more.',
		code: [
			'infra/csp_report.tf',
