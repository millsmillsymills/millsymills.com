...
4. **Pre-publish ACM validation CNAMEs at the existing registrar.** Targeting `millsymills.com`'s real cutover, the existing registrar is Squarespace (or whatever's authoritative right now per `dig NS millsymills.com`). The next step's `terraform apply` blocks on `aws_acm_certificate_validation.site`, which polls public DNS for two CNAMEs that AWS auto-generates — but those CNAMEs go into your Route53 zone, which is not yet authoritative. Without this step, ACM validation hangs until you cut over, but you can't cut over until apply finishes. Break the deadlock by running `./scripts/tf.sh <stack> apply -target=aws_acm_certificate.site -target=aws_route53_record.cert_validation` first, then read the two `_xxx.<domain>` CNAME pairs from `aws acm describe-certificate --region us-east-1 --certificate-arn $(terraform -chdir=infra output -raw acm_certificate_arn) --query 'Certificate.DomainValidationOptions[].ResourceRecord'` (or the AWS console), and publish them at the existing registrar's DNS. ~5 min later ACM validation completes and you can proceed to step 5. **The p41m0n.com rehearsal surfaced this gap** — without it, apply hangs ~25 min until your AWS STS token expires, then you debug a credential failure that's actually a DNS chicken-and-egg.
