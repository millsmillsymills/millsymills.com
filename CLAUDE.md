...
- **Branch protection on `main` is NOT Terraform-managed.** It is governed by the `default-branch-protection` repository ruleset (added when the repo went public, #634/#660; `enforcement: active`) plus out-of-band `required_signatures` — both managed in the GitHub UI. The former `infra/github_branch_protection.tf.disabled` (a parked `github_branch_protection_v3` with a live `import` block) was deleted in #757 as superseded dead code; do not reintroduce a `github_branch_protection_*` resource without first reconciling its scoping against the live ruleset, or the two will fight.
