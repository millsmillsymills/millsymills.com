...
Findings here that should land in the millsymills cycle's planning before that work starts:

- **`protonmail-mcp` `x-pm-uid` fix.** `protonmail-mcp#2`. Required before the millsymills cycle, which needs `proton_create_address` for `mills@millsymills.com`.
- **Calibration of "wait 5–10 min" for verification TXT propagation.** Whatever the actual observed delay was here, record it. The millsymills cycle's runbook should use the calibrated value, not a guess.
- **Any unexpected TF plan diff** during round 1 or round 2 (e.g. CAA, DNSSEC, CloudFront drift) is a latent bug in the shared `infra/` and should be fixed in `infra/` before the millsymills cycle, the same way the rehearsal spec routes findings back into the runbook.
